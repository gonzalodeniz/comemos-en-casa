...
## Slice 3 — Task 5 GREEN progress

- [x] Task 5: extended `backend/src/comemos_en_casa/meal_calendar/repository.py` with shared recurrence rule lookup, candidate listing, idempotent insert/update/delete methods, virtual weekly occurrence expansion, stable `series:{series_id}:{occurrence_date}` identities, and deterministic combined ordering. The persisted checkbox in `tasks.md` is checked.
- Ordinary assignment reads retain the existing `LEFT JOIN recipes` projection, while recurrence candidates are filtered to `calendar_key = "shared"` and `initial_date <= week_end`. Virtual occurrences remain read-only projections and never write `meal_assignments`.
- Files changed in this slice: `backend/src/comemos_en_casa/meal_calendar/repository.py`, `openspec/changes/repetir-comidas-calendario/tasks.md`, and this cumulative progress artifact. The pre-existing uncommitted `verify-report.md` change was not included or modified.
- Structured status consumed: native `ready`, recommended action `apply`, OpenSpec artifact store, repo-local action context with workspace-root edit authority; the parent resolved the high-risk workload gate with the accepted size exception and assigned Task 5 only.
- Delivery boundary: slice 3, repository GREEN only. API routes, schemas, frontend, migrations, and unrelated tests were not edited.

## TDD Cycle Evidence — Task 5 GREEN

| Task | Test files | Layer | Safety net | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|---|---|
| 5. Repository persistence and combined reads | `backend/tests/meal_calendar/test_calendar_repository.py` | Repository unit tests with recording connection | 5 tests observed before implementation; 3 expected repository failures | Existing Task 4 RED run: `3 failed, 2 passed` in repository tests | Focused repository tests: `5 passed` | API compatibility run: `3 passed, 7 expected Task 6 RED failures`; malformed recurrence rows from the legacy API fake are ignored without affecting ordinary reads | Added shared projection metadata, centralized entry ordering, and unavailable-recipe ordering; repository tests remained `5 passed` |

### Verification commands

- `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=backend/src .venv/bin/pytest -q backend/tests/meal_calendar/test_calendar_repository.py` — `5 passed`.
- `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=backend/src .venv/bin/pytest -q backend/tests/meal_calendar/test_api_contract.py` — `3 passed, 7 failed`; remaining failures are the intentionally unimplemented Task 6 API recurrence contracts (`entryType` response wiring, recurrence writes, series routes, and series validation), not repository failures.

## Current remaining implementation tasks after Task 6
...
## Slice 4 — Task 6 GREEN progress

- [x] Task 6: exposed recurrence-aware assignment creation/conversion, discriminated `entryType` responses, series PATCH/DELETE routes, confirmation rules, idempotency conflict handling, validation, and transaction boundaries in `backend/src/comemos_en_casa/meal_calendar/api.py`.
- [x] Corrected the three API fixture/state setup gaps in `backend/tests/meal_calendar/test_api_contract.py`: the fixture now uses a stateful repository and transaction context, conversion seeds its ordinary source assignment, and series mutation tests seed their persisted rules. The existing ordinary-create expectation was aligned with the required `entryType` discriminator. Persisted Task 6 checkbox is checked in `tasks.md`.
- Files changed in this slice: `backend/src/comemos_en_casa/meal_calendar/api.py`, `backend/tests/meal_calendar/test_api_contract.py`, `openspec/changes/repetir-comidas-calendario/tasks.md`, `openspec/changes/repetir-comidas-calendario/apply-progress.md`, and the pending `openspec/changes/repetir-comidas-calendario/verify-report.md` artifact.
- Structured status consumed: native `ready`, recommended action `apply`, OpenSpec artifact store, repo-local workspace action context with `/opt/apps/comemos-en-casa` as the allowed edit root; the parent prompt resolved the high-risk workload gate with an accepted size exception for this chained slice.
- Delivery boundary: Task 6 API GREEN only; no frontend, migration, schema/service/repository, or unrelated test files were edited.

## TDD Cycle Evidence — Task 6 GREEN

| Task | Test files | Layer | Safety net | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|---|---|
| 6. API recurrence contract | `backend/tests/meal_calendar/test_api_contract.py` | FastAPI contract integration | `3 passed, 7 expected failures` before production changes | `7 failed, 3 passed` after the stateful fixture correction; failures were contract gaps, not collection errors | `10 passed` focused API contracts | Stateful create/retry/conflict, conversion, reanchor/immutable-slot, confirmed delete/idempotency, invalid non-mutating requests, and legacy recipe cases all passed; related repository/domain suite `22 passed` | Extracted discriminated response and series request/response helpers; API contracts remained `10 passed` |

### Verification commands

- `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=backend/src .venv/bin/pytest -q backend/tests/meal_calendar/test_api_contract.py` — GREEN, `10 passed, 2 warnings`.
