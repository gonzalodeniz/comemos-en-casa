...
- [ ] RED — add failing migration-contract tests under `backend/tests/recipes/test_repository.py` proving the catalogue migration creates `recipes(id uuid primary key, title, image_url, detail, title_search_key)`, is independent of calendar tables, and documents ownership/order.
